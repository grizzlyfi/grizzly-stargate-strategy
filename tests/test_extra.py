...
def test_losses_multiple_deposits_withdraw(
    ...
    print(f'strategy totalAssets: {strategy.estimatedTotalAssets()}')
    ...
    # Withdrawal user
    print(f'vaultShares user: {vault.balanceOf(user)}')
    vaultShares = vault.balanceOf(user)
...
def test_debt_0_harvest(
    ...
    assert token.balanceOf(user) > user_1_balance_before
    assert token.balanceOf(user) - user_1_balance_before > 200e18
